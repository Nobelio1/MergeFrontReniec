...
export class LoginComponent implements OnInit {
  ...
  login(): void {
    ...
    this.spinner.show();
    this.authService.login(authIn).subscribe((data: AuthOut) => {
      this.spinner.hide();
      this.authOut = data;
    }, error => {
      this.spinner.hide();
      ...
      if (this.authOut.code === this.environment.CODE_000) {
        console.log('antes de setear el token')
        console.log('token = ', this.authOut);
        this.seguridadService.setToken(this.environment.VAR_TOKEN, this.authOut.data);
        console.log('luego de setear el token')
  ...
  fnFirstTime(): void {
    this.utilService.getAlert('Aviso', 'El usuario es el DNI y la contraseña el DNI y luego debe cambiar la contraseña por una nueva de uso personal y no debe compartir por seguridad de la información.');
  }
