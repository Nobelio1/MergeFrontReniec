...
import {TipoSolicitud, TipoSolicitudOut} from "../../models/tipo-solicitud.model";
import {RegistroFirmasService} from "../../services/registro-firmas.service";
import {TipoArchivo, TipoArchivoOut} from "../../../../masters/models/maestro.model";
...
import {Archivo, DetalleSolicitud, RegistroFirmaIn, RegistroFirmaOut} from "../../models/firmas.model";
...
export class Step2DatosSolicitudComponent implements OnInit {
  ...
  @ViewChildren(Step2DetalleSolicitudComponent) components!: QueryList<Step2DetalleSolicitudComponent>;
  ...
  constructor(private formBuilder: FormBuilder,
              ...
              private oficinaService: OficinaService, public dialog: MatDialog,
  ...
  btnNext(stepper: MatStepper) {
    ...
    let arrayDetalle: DetalleSolicitud[] = component.map(value => value.setDetalleSolicitud());

    // VALIDACION 1
    if (arrayDetalle.length <= 0) {
      this.utilService.getAlert('Aviso', 'Debe añadir por lo menos un (1) detalle de solicitud de firma.');
      return;
    }
    ...
    component.forEach(x => {
      if (x.form.invalid) {
        ...
        this.utilService.getAlert('Aviso', `El detalle de solicitud de firma (${x.index}), no cumple con los datos requeridos.`);
    ...
    component.forEach(x => {
      if (x.detalleSolicitud.idTipoSolicitud === this.environment.TIPO_SOLICITUD_ALTA) {
        const arrAltaRequired = ['03', '04'];
        const result = arrAltaRequired.filter((value) => !x.detalleSolicitud.detalleArchivo.some((obj) => obj.codigoTipoArchivo === value));
        if (result.length > 0) {
          const missing = this.tipoArchivoDetalleAlta.filter((obj) => result.some((value) => value === obj.codigo));
          const list = missing.map(item => `<li>${item.descripcion}</li>`).join('');
          this.utilService.getAlert('Tipo solicitud: ALTA', `En el detalle de solicitud de firma (${x.index}), debe añadir los siguientes documentos obligatorios: <ul class="mt-3">${list}</ul>`, 'left');
          cumpleValidacionesArchivos = false;
          return;
        }
      }
      if (x.detalleSolicitud.idTipoSolicitud === this.environment.TIPO_SOLICITUD_ACTUALIZAR) {
        const arrActualizarRequired = ['09', '10'];
        const result = arrActualizarRequired.filter((value) => !x.detalleSolicitud.detalleArchivo.some((obj) => obj.codigoTipoArchivo === value));
        if (result.length > 0) {
          const missing = this.tipoArchivoDetalleActualizar.filter((obj) => result.some((value) => value === obj.codigo));
          const list = missing.map(item => `<li>${item.descripcion}</li>`).join('');
          this.utilService.getAlert('Tipo solicitud: ACTUALIZAR', `En el detalle de solicitud de firma (${x.index}), debe añadir los siguientes documentos obligatorios: <ul class="mt-3">${list}</ul>`, 'left');
          cumpleValidacionesArchivos = false;
          return;
        }
      }
    ...
    this.registroFirmaIn.codigoTipoArchivoSustento = this.arrayFilesSustento[0].fileTypeId;
    ...
    this.spinner.show();
    this.registroFirmasService.registroFirma(this.registroFirmaIn).subscribe((data: RegistroFirmaOut) => {
      ...
      if (this.registroFirmaOut.code !== this.environment.CODE_000) {
        this.utilService.getAlert(`Aviso:`, `${this.registroFirmaOut.message}`);
        return;
      }
