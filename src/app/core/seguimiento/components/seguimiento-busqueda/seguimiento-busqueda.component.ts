...
export class SeguimientoBusquedaComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.form = this.formBuilder.group({
      numeroSolicitud: ['', [Validators.minLength(12), Validators.maxLength(12), Validators.pattern('^[0-9]*$')]],
      fechaInicio: [''],
      fechaFin: [''],
    });
  ...
  getLista(e?: PageEvent): void {
    ...
    if (this.form.invalid) {
      this.utilService.getAlert('Aviso', 'Debe completar los datos de búsqueda correctamente.');
      return;
    }
    ...
    busquedaIn.fechaIni = fInicio ? formatDate(fInicio, 'yyyy-MM-dd', 'EN') : '';
    ...
    this.seguimientoService.listSolicitudes(busquedaIn).subscribe((data: BusquedaOut) => {
  ...
  btnVerArchivosRespuesta(numeroSolicitud: string): void {
    ...
    this.spinner.show();
    this.seguimientoService.getDocumentosRespuesta(numeroSolicitud).subscribe((data: DocumentosRespuestaOut) => {
      this.spinner.hide();
      ...
      if (this.documentosRespuestaOut.code !== this.environment.CODE_000) {
        this.utilService.getAlert(`Aviso:`, `${this.documentosRespuestaOut.message}`);
        return;
      }
